...
def test_reset_running_to_pending_restores_interrupted_rows(workspace_temp_dir: Path) -> None:
    db = workspace_temp_dir / "state.sqlite3"
    store = StateStore(db)
    try:
        job_id = "job_x"
        ...
        store.upsert_page_images(
            page.id,
            [(1, "https://i/1.jpg", str(workspace_temp_dir / "a.jpg"))],
        )
